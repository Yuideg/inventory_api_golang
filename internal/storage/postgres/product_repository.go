...
func (pgx *ProductIMP) Create(product model.Product) (pgconn.CommandTag, error) {
...
func (pgx *ProductIMP) GetById(barcode string) (*model.Product, error) {
	row := pgx.pgx.QueryRow(pgx.ctx, "SELECT * FROM product WHERE user_id = $1", barcode)
...
func (pgx *ProductIMP) Get() ([]model.Product, error) {
...
func (pgx *ProductIMP) Update(product *model.Product) (pgconn.CommandTag, error) {
...
func (pgx *ProductIMP) Delete(barcode string) error {
	_, err := pgx.pgx.Exec(pgx.ctx, "DELETE FROM product WHERE barcode=$1", barcode)
	if err != nil {
