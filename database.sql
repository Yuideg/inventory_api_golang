...
create table stock (
    id  uuid  DEFAULT uuid_generate_v4 () ,
    products_code varchar(70) not null,
...
create table orders(
    ...
    customer_id uuid  not null ,CONSTRAINT fk_customer_id FOREIGN KEY(customer_id)
    REFERENCES customer(id) on update cascade  on delete cascade ,
    product_id uuid  not null, CONSTRAINT fk_product_barcode FOREIGN KEY(product_id)
...
-- ALTER TABLE customer ADD CONSTRAINT user_username UNIQUE (username);
-- ALTER TABLE customer ADD CONSTRAINT user_email UNIQUE (hashed_email);
-- CREATE INDEX user_username_idx ON customer (username, hashed_email,"password");
--
